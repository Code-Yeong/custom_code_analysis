...
class AvoidUsingShowGeneralDialog extends Rule {
  ...
  static const ruleId = 'avoid-using-show-general-dialog';
  static const code = ruleId;
  static const methodName = 'showGeneralDialog';
  static const comment = '快速添加';
  static const message = '禁止使用原生$methodName方法';
  static const correction = '原生$methodName函数禁止调用';

  AvoidUsingShowGeneralDialog(this._compilationUnit, this.analysisResult) {
...
class _MirrorVisitor extends RecursiveAstVisitor<void> {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    // logUtil.info('lineNumber = ${lineNumber}, ignored = ${_ignores.isSuppressedAt(AvoidUsingShowGeneralDialog.ruleId, lineNumber)}');
    if (node.methodName.name.contains(AvoidUsingShowGeneralDialog.methodName)) {
      int lineNumber = _compilationUnit.lineInfo.getLocation(node.offset).lineNumber;
      if (!_ignores.isSuppressedAt(AvoidUsingShowGeneralDialog.ruleId, lineNumber)) {
        _nodes.add(node);
      }
    }
  }
