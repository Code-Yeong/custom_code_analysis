...
class _MirrorVisitor extends RecursiveAstVisitor<void> {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    // logUtil.info('lineNumber = ${lineNumber}, ignored = ${_ignores.isSuppressedAt(AvoidUsingShowTimePicker.ruleId, lineNumber)}');
    if (node.methodName.name.contains(AvoidUsingShowTimePicker.methodName)) {
      int lineNumber = _compilationUnit.lineInfo.getLocation(node.offset).lineNumber;
      if (!_ignores.isSuppressedAt(AvoidUsingShowTimePicker.ruleId, lineNumber)) {
        _nodes.add(node);
      }
    }
  }
