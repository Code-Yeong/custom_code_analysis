...
class AvoidUsingShowSearch extends Rule {
  ...
  static const ruleId = 'avoid-using-show-search';
  static const code = ruleId;
  static const methodName = 'showSearch';
  static const comment = '快速添加';
  static const message = '禁止使用原生$methodName方法';
  static const correction = '原生$methodName函数禁止调用，请使用xxx';

  AvoidUsingShowSearch(this._compilationUnit, this.analysisResult) {
...
class _MirrorVisitor extends RecursiveAstVisitor<void> {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    // logUtil.info('lineNumber = ${lineNumber}, ignored = ${_ignores.isSuppressedAt(AvoidUsingShowSearch.ruleId, lineNumber)}');
    if (node.methodName.name.contains(AvoidUsingShowSearch.methodName)) {
      int lineNumber = _compilationUnit.lineInfo.getLocation(node.offset).lineNumber;
      if (!_ignores.isSuppressedAt(AvoidUsingShowSearch.ruleId, lineNumber)) {
        _nodes.add(node);
      }
    }
  }
